...
A real-time drone simulator with 3D visualization, interactive map, and flight controls. Connects to ArduPilot SITL via MAVLink and presents a web-based dashboard with Three.js 3D rendering, Leaflet map tracking, console logging, and command controls.
...
Browser (index.html)
    |
    +-- Three.js 3D scene (drone model, ground, trail, sky)
...
- **`demo_roundtrip` method**: a complete flight sequence (arm, takeoff to 50m, fly 100m north, land, wait 25s, re-arm, takeoff to 75m, fly back, land). Uses `wait_altitude`, `wait_position`, and `wait_disarmed` helpers with configurable timeouts and progress callbacks. On any exception, attempts to set LAND mode as a safety fallback.
...
**Key design decisions:**
- **Three.js r149** (not r160+). Version r160 removed the `examples/js/` directory (OrbitControls.js, etc.) in favor of ES modules. Using r149 allows loading OrbitControls via a simple `<script>` tag from unpkg CDN without a module bundler.
...
Central configuration file read by both the backend (config_loader.py) and the SITL launch script (start_sitl.sh).
...
Version r160 (August 2023) removed the legacy `examples/js/` directory, requiring ES module imports for OrbitControls. Since the dashboard is a single HTML file served by Flask (no bundler), using `<script>` tags from CDN is the simplest approach. Three.js r149 is the last version supporting `examples/js/controls/OrbitControls.js` via script tag.
